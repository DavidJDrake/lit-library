# Lit Library Web Front End Implementation Plan (Plan 3 of 3)

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build and deploy the user-facing single-page app at `https://lit.example.com`: Google sign-in through Cognito, a browsable/searchable/filterable card grid of the 1,164-book catalog with covers, a detail view with per-format download buttons that call the deployed `POST /download` API, a clear "private library" message for rejected sign-ins, and minimal `/privacy` and `/terms` pages.
...
- Rejected sign-ins arrive as `?error=…&error_description=PreSignUp+failed+with+error+This+is+a+private+library…`; the app must display the human message (strip the `PreSignUp failed with error ` prefix).
...
- Produces: `AuthProvider({ config, children, fetchFn?, navigate? })` and `useAuth(): AuthState` where
  ...
  interface AuthState {
    ...
    email?: string;
    error?: string;                    // access-denied / callback error to display
...
6. In a private window, sign in with a non-allowlisted Google account → the sign-in page shows the red banner "This is a private library. Access is limited to authorized accounts".
